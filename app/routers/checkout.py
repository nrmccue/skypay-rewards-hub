...
# Import USERS from wallet module for demo purposes
...
def checkout(req: CheckoutRequest):
    ...
    if req.use_points:
        if req.points_to_use is None:
            ...
            points_redeemed = max(0, min(user["points"], int(req.points_to_use)))
            # cap at 80%
            max_pts = int((0.8 * subtotal_usd) / POINT_VALUE_USD)
            points_redeemed = min(points_redeemed, max_pts)
